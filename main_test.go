...
func TestVersionLdflags(t *testing.T) {
	workdir, err := os.MkdirTemp("", "gopy-version-ldflags-")
	if err != nil {
		t.Fatalf("could not create workdir: %v\n", err)
	}
	defer os.RemoveAll(workdir)
	...
	ldflags := fmt.Sprintf("-X 'main.GitCommit=%s' -X 'main.VersionDate=%s'", wantCommit, wantDate)

	build := exec.Command("go", "build", "-ldflags", ldflags, "-o", binPath, ".")
	if out, err := build.CombinedOutput(); err != nil {
		t.Fatalf("could not build with ldflags: %+v\n%s", err, out)
	}

	out, err := exec.Command(binPath, "version").CombinedOutput()
	if err != nil {
		t.Fatalf("could not run %s version: %+v\n%s", binPath, err, out)
	}
	got := strings.TrimSpace(string(out))
	want := fmt.Sprintf("gopy version %s (commit %s, built %s)", Version, wantCommit, wantDate)
	if got != want {
		t.Fatalf("gopy version (ldflags-stamped) = %q, want %q", got, want)
	}
}

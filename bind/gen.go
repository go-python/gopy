...
# from %[1]s import %[1]s
# and then refer to everything using %[1]s. prefix
# (if part of a larger package, add package name prefixes in the from part)
...
func (g *pyGen) genPyBuildPreamble(cmd string) {
	n := g.pkg.pkg.Name()
	pkgimport := g.pkg.pkg.Path()
	g.pybuild.Printf(PyBuildPreamble, n, pkgimport, cmd)
}

func (g *pyGen) genPyWrapPreamble(cmd string) {
	n := g.pkg.pkg.Name()
	pkgimport := g.pkg.pkg.Path()
...
func (g *pyGen) genMakefile(cmd string) {
	n := g.pkg.pkg.Name()
	pkgimport := g.pkg.pkg.Path()

...
`
	...
	// same argument positions as MakefileTemplate, though only 1-5 and 10 are used:
	// cffi needs none of the CPython CFLAGS/LDFLAGS that building %[1]s.c would.
	MakefileTemplateCFFI = `# Makefile for python interface for package %[1]s, using cffi.
# File is generated by gopy version %[10]s. Do not edit.
# %[2]s
...
build:
	...
	$(GOIMPORTS) -w %[1]s.go
	# generate %[1]s_go$(LIBEXT) from %[1]s.go -- the cgo wrappers to go functions.
	# unlike the default (pybindgen) backend, this is the only library gopy
	# builds: cffi loads it directly, with no Python.h/libpython involved.
	$(GOBUILD) -buildmode=c-shared -o %[1]s_go$(LIBEXT) %[1]s.go
	# writes _%[1]s.py, the cffi module %[1]s.py imports
	$(PYTHON) build.py
